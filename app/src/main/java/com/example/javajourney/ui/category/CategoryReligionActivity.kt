...
class CategoryReligionActivity : AppCompatActivity() {
    ...
    private fun fetchCategoryAlamData() {
        ...
        val call = apiService.getCategoryAlam("Wisata Religi")
        ...
        call.enqueue(object : Callback<GetPlace> {
            override fun onResponse(call: Call<GetPlace>, response: Response<GetPlace>) {
                ...
                if (response.isSuccessful) {
                    ...
                    val places = body?.religion
                    if (places != null && places.isNotEmpty()) {
                        adapter.updateData(places)
                    } else {
                        Log.d("CategoryAlamActivity", "No places found or list is empty")
                        Toast.makeText(this@CategoryReligionActivity, "No places found", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    val errorBody = response.errorBody()?.string()
                    Log.d("CategoryAlamActivity", "Failed to retrieve data: $errorBody")
                    Toast.makeText(this@CategoryReligionActivity, "Failed to retrieve data", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onFailure(call: Call<GetPlace>, t: Throwable) {
                ...
                Toast.makeText(this@CategoryReligionActivity, "Request failed: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
}
